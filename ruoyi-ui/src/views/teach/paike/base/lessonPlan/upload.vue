<template>
  <el-dialog :title="upload.title" :visible="true" width="400px"
    ...
    <el-upload ref="upload" :limit="1" accept=".xlsx, .xls"
               :headers="upload.headers" :action="url + '/importData?updateSupport=' + upload.updateSupport"
...
<script>
import service from "@/api/teach/paike/base/lessonPlan"
...
export default {
  ...
  methods: {
    ...
    importTemplate() {
      // window.open(process.env.VUE_APP_BASE_API + '/data/teach/paike/lessonPlan.xlsx',{},'任课信息导入模板.xlsx');
      window.open(process.env.VUE_APP_BASE_API + '/data/teach/paike/lessonPlan.xlsx');
    },
  }
}
</script>
